...
class VehicleOnEarthSurface:
    ...
    def recalc_speed( self, time_s, throttle_ctrl, brake_ctrl ):
        ...
        throttle_ctrl - input from user, float -1 to 1, indicates how much acceleration is applied,
                        negative value means rearing
        brake_ctrl - input from user, float 0 to 1, indicates how much braking is applied"""

        self.speed_m_s += self.max_thrust_n * throttle_ctrl / self.mass_kg * time_s

        if self.speed_m_s == 0:
            return

        deceleration_m_s = ( self.internal_friction_n
                           + self.drag_compontents * self.speed_m_s**2
                           + self.max_braking_n * brake_ctrl ) / self.mass_kg * time_s

        if self.speed_m_s > 0:
            self.speed_m_s -= deceleration_m_s
            if self.speed_m_s < 0:
                self.speed_m_s = 0
        else:
            self.speed_m_s += deceleration_m_s
    ...
    def move_straight( self, dist_m ):
        ...
        if dist_m < 0:
            movement_deg = ( self.bearing_deg + 180 ) % 360
        else:
            movement_deg = self.bearing_deg

        prev_lat, prev_lng = self.lat, self.lng
        self.lat, self.lng = earth_walk.step( self.lat, self.lng, movement_deg,
                            earth_walk.Earth_dist_to_deg( abs( dist_m ) ) )

        self.bearing_deg = earth_walk.dist_and_brng( self.lat, self.lng, prev_lat, prev_lng )[1]

        if dist_m > 0:
            self.bearing_deg = ( self.bearing_deg + 180 ) % 360
    ...
    def move_curved( self, dist_m, steering_ctrl ):
        ...
        angle_turned_rad = abs( dist_m ) / turn_radius_m
        ...
        if dist_m < 0:
            ...
            new_bearing_deg = ( earth_walk.dist_and_brng( new_lat, new_lng, self.lat, self.lng )[1]
    ...
    def move( self, time_s, steering_ctrl, throttle_ctrl, brake_ctrl ):
        ...
        steering_ctrl - float from -1 to 1, negative - left, positive - right
        throttle_ctrl - float from -1 to 1
